######################################################
## REVIEW SIMULATIONS: RESULTS WITH 100 SCANS ##
######################################################
...
for(i in 1:nrow(conditions_results)) {
	...
	for(s in 1:nsim) {
		...
		#s<- 1
		setwd(simulations)
		tmap <- f.read.nifti.volume(paste("TMAP_Multiple_null_simulation_",s,"_noise_",sigmnoise,"_scans_100.nii", sep=""))[,,,1]
		tmap <- tmap*noisemask
		sb1 <- f.read.nifti.volume(paste("SB1_Multiple_simulation_",s,"_noise_",sigmnoise,"_scans_100.nii", sep=""))[,,,1]
		sb1 <- sb1*noisemask
	...
	setwd(results)
	write.csv(tmp.counts.act, paste("ALL_COUNTS_ACTIVE_100_noise_",sigmnoise,"_alpha_",alpha,"_beta_",beta,sep=""))
	write.csv(tmp.counts.inact, paste("ALL_COUNTS_INACTIVE_100_noise_",sigmnoise,"_alpha_",alpha,"_beta_",beta,sep=""))
	write.csv(tmp.counts.unc, paste("ALL_COUNTS_UNCERTAIN_100_noise_",sigmnoise,"_alpha_",alpha,"_beta_",beta,sep=""))
	write.csv(tmp.counts.practins, paste("ALL_COUNTS_PRACTINS_100_noise_",sigmnoise,"_alpha_",alpha,"_beta_",beta,sep=""))
...
setwd(results)
write.csv(results.counts.act, "COUNTS_ACTIVE_100")
write.csv(results.counts.inact, "COUNTS_INACTIVE_100")
write.csv(results.counts.unc, "COUNTS_UNCERTAIN_100")
write.csv(results.counts.practins, "COUNTS_PRACTINS_100")
